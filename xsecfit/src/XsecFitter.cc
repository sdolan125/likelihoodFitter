...
// PrepareFitter
void XsecFitter::InitFitter(std::vector<AnaFitParameters*> &fitpara, Double_t reg)
{
...
void XsecFitter::Fit(std::vector<AnaSample*> &samples, int datatype)
{
  ...
  if(datatype==1){
    ...
    for(size_t s=0;s<m_samples.size();s++)
      {
        //((AnySample*)m_samples[s])->GetSampleId(); 
        //((AnySample*)m_samples[s])->GetSampleBreakdown();
        ((AnySample*)m_samples[s])->GetSampleBreakdown(m_dir,((string)"thrown"));
      }
  }
    ...
    for(size_t s=0;s<m_samples.size();s++)
    {
      ...
      //((CCQESample*)m_samples[s])->GetSampleBreakdown(m_dir,"thrown");
    }
  }
  ...
  for(size_t i=0;i<m_fitpara.size();i++)
    {
      ...
      for(int j=0;j<m_nparclass[i];j++){
        ...
        if(i==0 && j!=0) chi2_reg+= reg_p1*(par[k-1]-par[k-2])*(par[k-1]-par[k-2]);
        cout << "param " << j << " of class " << i << " has value " << par[k-1] << " giving total chi2_reg " << chi2_reg << endl;
      }
      chi2_sys += m_fitpara[i]->GetChi2(vec);
      new_pars.push_back(vec);
      cout << "chi2_sys contribution from param set " << i << " is " << m_fitpara[i]->GetChi2(vec) << endl; 
    }
  cout << "chi2_sys contribution from regularisation " << chi2_reg << endl;
  chi2_sys += chi2_reg;
  vec_chi2_sys.push_back(chi2_sys);
...
void XsecFitter::DoSaveResults(vector< vector<double> > parresults, vector< vector<double> > parerrors, double chi2){
  ...
  for(size_t s=0;s<m_samples.size();s++)
    {
      ((AnySample*)m_samples[s])->GetSampleBreakdown(m_dir,"fit");
    }
}
